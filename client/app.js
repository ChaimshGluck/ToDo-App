...
async function getTasks(userid) {
    let response = await fetch(`http://localhost:3000/tasks?userid=${userid}`, {
        headers: {
...
async function addTaskBtnClicked() {
    if (!document.querySelector("#new-task").value) return;
    let newTaskObject = {
...
function displayTasks() {
    ...
    todoSectionElement.innerHTML = `<h2 class="task-header">To do - <span id="todo-count"></span></h2>`;

    let doneSectionElement = document.querySelector("#done-list");
    doneSectionElement.innerHTML = `<h2 class="task-header">Done - <span id="done-count"></span></h2>`;
    ...
    document.querySelector("#done-count").innerText = doneCount;
    document.querySelector("#todo-count").innerText = todoCount;
};
...
function buildTaskElement(task) {
  ...
  ${task.done ? "" :
      ...
      <button onclick="promptDeleteWarning(this)">
          <svg
              width="19"
              height="24"
              viewBox="0 0 25 24"
...
async function markAsDone(thisObject) {
    let id = thisObject.parentNode.parentNode.getAttribute('data-task-id');
    await fetch(`http://localhost:3000/tasks/${id}`, {
        method: "PATCH"
    })
    tasks.find((task) => Number(task.id) === parseInt(id)).done = true;
    displayTasks();
};

function promptDeleteWarning(thisObject) {
    const taskElement = thisObject.closest('.task');
    const taskId = taskElement.getAttribute('data-task-id');
    const deleteBox = document.querySelector("#deleteBox");
    deleteBox.setAttribute('data-task-id', taskId);
...
async function confirmDelete() {
    ...
    let id = document.querySelector("#deleteBox").getAttribute('data-task-id');

    await fetch(`http://localhost:3000/tasks/${id}?username=${userState.username}&pass=${userState.pass}`, {
        method: "DELETE"
    });
