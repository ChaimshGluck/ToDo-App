...
export async function createUser(body) {
    ...
    const result = await db.one('insert into todos1.person (username, email, pass, firstname, lastname) values (${username}, ${email}, ${pass}, ${firstName}, ${lastName}) returning *', {
...
export async function userLogin(query) {
    let users = await db.manyOrNone('select firstname, username, pass, id from todos1.person where username = $1', [query.username]);
    if (!users) return { ok: false };
    for (let user of users) {
        let userExists = await bcrypt.compare(query.pass, user.pass);
        if (userExists) return { ok: true, id: user.id, status : user.status };
    }
    return { ok: false };
};
