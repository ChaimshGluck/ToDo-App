...
export default function Task({ task, loadTasks }) {
    console.log('Task:', task);
    const [showDeleteBox, setShowDeleteBox] = useState(false);
    const taskToDelete = useRef(null);
    ...
    return (
        <div className="task-container">
            <div className="task-title-and-img">
                {task.image && <img src={`http://localhost:3000/${task.image}`} alt="Task" width="250" height="150" />}
                <p>{task.title}</p>
            </div>
            <div id='buttonContainerStyle'>
                {!task.done &&
                    <button className='buttonStyle' onClick={markAsDone}>
                ...
                <button className='buttonStyle' onClick={() => deleteTask(task.id)}>
                    <svg className='iconStyle'
                        width="19"
                        height="24"
                        viewBox="0 0 25 24"
            ...
            {showDeleteBox &&
                <DeleteWarning taskToDelete={taskToDelete} setShowDeleteBox={setShowDeleteBox} loadTasks={loadTasks} />
            }
        </div>
    );
}